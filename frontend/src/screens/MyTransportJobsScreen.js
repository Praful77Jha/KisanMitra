...
import { canUpdateJobStatus } from '../utils/transportStatus';
...
export default function MyTransportJobsScreen() {
  ...
  const load = useCallback(() => {
    ...
    setLoading(true);
    setError('');
    return Promise.all([getTransportJobs(), getMyTransportRequests()])
      .then(([jobList, requestList]) => {
        if (cancelled) return;
        setJobs((jobList || []).filter((j) => canUpdateJobStatus(j.status)));
        const map = {};
  ...
  const refresh = useCallback(() => {
    setRefreshing(true);
    Promise.all([getTransportJobs(), getMyTransportRequests()])
      .then(([jobList, requestList]) => {
        setJobs((jobList || []).filter((j) => canUpdateJobStatus(j.status)));
        const map = {};
        (requestList || []).forEach((request) => {
  ...
  return (
    <View style={styles.screen}>
      <Header
        title={requester ? t('transport.myJobs') : t('transport.activeJobs')}
        showBack
